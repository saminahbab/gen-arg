...
def main():
    ...
    # Required parameters
    parser.add_argument(
        "--model", type=str, required=True, choices=["gen", "constrained-gen"]
    )
    parser.add_argument(
        "--dataset", type=str, required=True, choices=["RAMS", "ACE", "KAIROS"]
    )
    ...
    parser.add_argument(
        "--sample-gen", action="store_true", help="Do sampling when generation."
    )
    parser.add_argument(
        "--train_batch_size",
        default=8,
    ...
    parser.add_argument(
        "--gradient_clip_val", default=1.0, type=float, help="Max gradient norm."
    )
    parser.add_argument(
        "--num_train_epochs",
        default=3,
    ...
    if args.load_ckpt:
        model.load_state_dict(
            torch.load(args.load_ckpt, map_location=model.device)["state_dict"]
        )
