    ...
    def create_gold_gen(
        ...
        if len(context_words) > MAX_CONTEXT_LENGTH:
            ...
            if cur_len > MAX_CONTEXT_LENGTH:
                ...
                end_idx = min(
                    len(context_words), trigger_start + MAX_CONTEXT_LENGTH // 2
                )
                ...
                while cur_len > prev_len:
                    ...
                    if right < total_sents:
                        right_sent_tokens = [
                            tup[0] for tup in ex["sentences"][right][0]
                        ]
        ...
        if mark_trigger:
            ...
            tgt = self.tokenizer.tokenize(
                " ".join(context_words[trigger["start"] : trigger["end"]]),
        ...
        if not os.path.exists(data_dir):
            ...
            for split, f in [
                ...
                with open(f, "r") as reader, open(
                    os.path.join(data_dir, "{}.jsonl".format(split)), "w"
                ) as writer:
                    for line, coref_line in zip(reader, coref_reader):
                        ...
                        for i in range(len(ex["event_mentions"])):
                            if (
                                split == "train"
                                and len(ex["event_mentions"][i]["arguments"]) == 0
                            ):
                            ...
                            if (
                                evt_type not in ontology_dict
                            ):  # should be a rare event type
                                continue
